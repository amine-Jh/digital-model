...
2. Run **`0002_` … `0005_` …** in numeric order if your baseline predates those patches (most are idempotent; skip any that already match your DB).
...
- Prefer applying only the **incremental** files you are missing (e.g. `0005_geometry_session_analytics.sql`), not re-applying `0001` wholesale.
...
- add a **new** numbered migration (`0008_…`) for production and leave `0001` as the last merged snapshot you trust.

## Template

Copy **`9999_TEMPLATE.sql.example`** to the next free number (e.g. `0006_your_change.sql`), replace the placeholder, and open a PR.
